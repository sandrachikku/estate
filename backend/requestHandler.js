...
import soldproductSchema from './models/soldproduct.js'
...
export async function addaddress(req, res){
  const { houseName, pincode, postOffice, place } = req.body;
  if (houseName && pincode && postOffice && place) {
    // Save address to the database
    // Example: db.saveAddress({ houseName, pincode, postOffice, place })
    res.status(201).json({ message: 'Address added successfully', address: { houseName, pincode, postOffice, place } });
  } else {
    res.status(400).json({ message: 'Invalid address details' });
  }
}
